...
from ProblemSpec_0928 import EX7   # or EX7 if you want to switch
...
def run_once(device: torch.device):
    ...
    """
    spec = EX7()  # choose your problem (EX5 in user snippet)
    ...
    num_epochs = 250
    x_domain = (-1.0, 1.0)
    x0_mode  = ("fixed", {"x_min": None, "x_max": None})  # sampling for each segment's initial x
    initial_x = 0.1                                        # earliest segment starts here
    ...
    u_hat, u_se = recover_u_multi(
        ...
        num_a_points=100, time_steps_per_segment=6, n_paths=10000, device=device
    )
...
def main():
    ...
    try:
        ...
        df.to_csv(f"multi_period_batch_EX7_T08_{ts}.csv", index=False)
        print(f"Saved per-run metrics to_{ts}.csv")
