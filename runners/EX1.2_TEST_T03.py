...
from ProblemSpec_0928 import (ProblemSpec, EX7, EX5, EX4)

# ------------------------------ Config ------------------------------
N_RUNS = 50

# Problem + training hyperparams (same as your example)
SPEC_CTOR = EX5                      # pick EX7/EX5/EX4
T = 0.3
TIME_STEPS = 16
TRAIN_PATHS = 5000
BATCH_SIZE = 5000
...
def main():
    ...
    print(f"SD ( rel error )      = {std_rel_err:.6e} (sample SD)")
    ...
    try:
        ...
        df.to_csv(f"single_period_batch_EX5_T03{SPEC_CTOR.__name__}_{ts}.csv", index=False)
